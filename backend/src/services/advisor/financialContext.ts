...
export type MonthlyTotals = { month: string; income: number; expense: number };

export type TopCategory = { categoryId: string; categoryName: string | null; total: number };

export type GoalProgress = { id: string; title: string; targetAmount: string; currentAmount: string; progressPercent: number; daysRemaining: number | null };
...
export type FinancialContext = {
  ...
  subscriptions: { name: string; amount: string; frequency: string }[];
  totalSubscriptionMonthly: string;
  largestExpense: { amount: string; description?: string } | null;
  averageSavingRatePercent: number;
};

const currencyFormatter = new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" });

export async function buildUserFinancialContext(userId: string, periodMonths = 3): Promise<FinancialContext> {
  const now = new Date();
  ...
  const totalsByMonth: MonthlyTotals[] = Array.from(monthMap.entries())
    .sort()
  ...
  const topExpenseCategories: TopCategory[] = Array.from(expenseByCategory.entries())
    .map(([cid, total]) => ({ categoryId: cid, categoryName: categoryNames.get(cid) ?? null, total }))
    .sort((a, b) => b.total - a.total)
    .slice(0, 5);

  // goals progress
  const goals = await prisma.goal.findMany({ where: { userId, status: { not: "CANCELLED" } }, include: { movements: true } });

  const activeGoals: GoalProgress[] = goals.map((g) => {
  ...
  const subsFormatted = subscriptions.map((s) => {
    ...
    if (s.frequency === "WEEKLY") monthly = monthly * (52 / 12);
    if (s.frequency === "YEARLY") monthly = monthly / 12;
  ...
  let largestExpense = null as FinancialContext["largestExpense"];
