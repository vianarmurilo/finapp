/**
 * Auto categorizador usando API Anthropic (claude-sonnet-4-20250514)
 */
type Category = { id: string; name: string };

export async function suggestCategoriesBatch(descriptions: string[], categories: Category[]): Promise<(string | null)[]> {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) throw new Error("Chave Anthropic (ANTHROPIC_API_KEY) nao configurada");
  ...
  for (let i = 0; i < descriptions.length; i += batchSize) {
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        ...
        for (const ln of lines) results.push(ln.trim() || null);
      }
    } catch (err) {
      const lines = text.split(/\r?\n/).filter(Boolean);
      for (const ln of lines) results.push(ln.trim() || null);
...
function buildPrompt(descriptions: string[], categories: Category[]) {
