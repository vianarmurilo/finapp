...
exports.normalizeDocumentType = normalizeDocumentType;
exports.normalizePaymentMethod = normalizePaymentMethod;
exports.validateExtractedReceipt = validateExtractedReceipt;
...
function normalizePaymentMethod(value) {
    ...
    if (!text)
        return null;
    if (text === "dinheiro")
        return receipt_1.PaymentMethod.Dinheiro;
    if (text === "débito" || text === "debito")
        return receipt_1.PaymentMethod.Debito;
    if (text === "crédito" || text === "credito")
        return receipt_1.PaymentMethod.Credito;
    if (text === "pix")
        return receipt_1.PaymentMethod.Pix;
    return null;
}
function normalizeDocumentType(value) {
    const text = String(value ?? "").trim().toLowerCase();
    if (text === receipt_1.DocumentType.NFCe)
        return receipt_1.DocumentType.NFCe;
    if (text === receipt_1.DocumentType.CupomFiscal)
        return receipt_1.DocumentType.CupomFiscal;
    if (text === receipt_1.DocumentType.NotaPaulista)
        return receipt_1.DocumentType.NotaPaulista;
    if (text === receipt_1.DocumentType.NotaFiscal)
        return receipt_1.DocumentType.NotaFiscal;
    return receipt_1.DocumentType.Desconhecido;
}
...
function buildUserPrompt(categories, base64, mimeType) {
...
function validateExtractedReceipt(raw, allowedCategories) {
    ...
    if (documentType === receipt_1.DocumentType.Desconhecido) {
        throw new app_error_1.AppError("Não foi possível identificar uma nota fiscal nesta imagem", 422);
    }
    if (!establishmentName || !date || !Number.isFinite(totalAmount)) {
        throw new app_error_1.AppError("Não foi possível ler a nota fiscal desta imagem. Tente uma foto mais nítida.", 422);
    }
...
async function anthropicExtract(imageBase64, mimeType, userId) {
    ...
    if (!apiKey) {
        throw new app_error_1.AppError("Chave da Anthropic não configurada", 500);
    }
    ...
    try {
        ...
        if (!response.ok) {
            throw new app_error_1.AppError("Não foi possível ler a nota fiscal no momento. Tente novamente.", 422);
        }
        ...
        if (error instanceof Error && error.name === "AbortError") {
            throw new app_error_1.AppError("A leitura da nota demorou demais. Tente novamente em instantes.", 504);
        }
...
function enforceDailyScanLimit(userId) {
    const { start, end } = getTodayRange();
    return prisma_1.prisma.receiptScan.count({
        where: { userId, createdAt: { gte: start, lt: end } },
    }).then((count) => {
        if (count >= MAX_SCANS_PER_DAY) {
            throw new app_error_1.AppError("Você atingiu o limite diário de leituras de nota fiscal.", 429);
        }
    });
}
/**
 ...
 */
async function extractReceiptData(imageBase64, mimeType, userId) {
    if (!imageBase64.trim()) {
        throw new app_error_1.AppError("Imagem inválida.", 422);
    }
    ...
    const allowedCategories = (await prisma_1.prisma.category.findMany({
        where: { userId },
        orderBy: { name: "asc" },
        select: { name: true },
    })).map((category) => category.name);
    const data = validateExtractedReceipt(parsed, allowedCategories);
    const scan = await prisma_1.prisma.receiptScan.create({
        data: {
            userId,
 ...
 */
async function confirmReceipt(userId, request) {
    const scan = await prisma_1.prisma.receiptScan.findFirst({
        where: { id: request.scanId, userId },
    });
    if (!scan) {
        throw new app_error_1.AppError("Leitura de nota fiscal não encontrada.", 404);
    }
    if (scan.transactionId) {
        throw new app_error_1.AppError("Esta leitura já foi confirmada.", 409);
    }
    const transactionService = new transaction_service_1.TransactionService(new transaction_repository_1.TransactionRepository(), new category_repository_1.CategoryRepository());
    ...
    await prisma_1.prisma.receiptScan.update({
        where: { id: scan.id },
